...
`, os.Args[0])
	...
	cfgetmergeOpts = getopt.New()
	cfgetmergen    = cfgetmergeOpts.Bool('n')

	dfOpts = getopt.New()
	dfh    = dfOpts.Bool('h')

	s3Opts     = getopt.New()
	s3ak       = s3Opts.StringLong("access_key", 'a', "", "s3 access key")
	s3sk       = s3Opts.StringLong("secret_key", 's', "", "s3 secret key")
	s3bucket   = s3Opts.StringLong("bucket", 'b', "", "s3 bucket like s3://test_bucket or s3://test_bucket/test_folder/test_key")
	s3endpoint = s3Opts.StringLong("endpoint", 'e', "", "s3 endpoint url like s3test.sumeru.mig")
	workerNums = s3Opts.UintLong("workers", 'n', 10, "upload workers")
	exitQuick  = s3Opts.BoolLong("exitquick", 'q', "exit quickly when one of key error")
...
func init() {
	...
	cfgetmergeOpts.SetUsage(printHelp)
	dfOpts.SetUsage(printHelp)
	s3Opts.SetUsage(printHelp)
}
...
func main() {
	...
	case "cfgetmerge":
		cfgetmergeOpts.Parse(argv)
		cfgetmerge(cfgetmergeOpts.Args(), *cfgetmergen)
	case "put":
		put(argv[1:])
	case "s3":
		s3Opts.Parse(argv)
		s3put(s3Opts.Args(), *s3ak, *s3sk, *s3bucket, *s3endpoint, *workerNums, *exitQuick)
	case "df":
		dfOpts.Parse(argv)
		df(*dfh)
